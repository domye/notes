...
### [线程有几种状态？](https://javabetter.cn/sidebar/sanfene/javathread.html#_6-🌟线程有几种状态)
...
![三分恶面渣逆袭：Java线程状态变化](https://cdn.paicoding.com/tobebetterjavaer/images/sidebar/sanfene/javathread-7.png)三分恶面渣逆袭：Java线程状态变化
...
- `sleep()`：是 `Thread` 类的**静态方法**，直接通过 `Thread.sleep(毫秒数)` 调用。
- `wait()`：是 `Object` 类的**成员方法**，必须通过对象（通常是锁对象）调用，如 `obj.wait()`。

### 锁的处理（最关键区别）

- `sleep()`：**不会释放持有的锁**。线程调用 `sleep` 后进入休眠状态，但依然霸占着锁，其他线程无法获取该锁。
- `wait()`：**会主动释放持有的锁**。线程调用 `wait` 后，会释放锁并进入该对象的 “等待池”，其他线程可以获取锁执行，直到被 `notify()`/`notifyAll()` 唤醒。

###  唤醒方式

- `sleep()`：有两种唤醒方式：① 休眠时间到自动唤醒；② 被其他线程调用 `interrupt()` 中断（会抛出 `InterruptedException`）。
- `wait()`：① 被其他线程调用 `obj.notify()`/`obj.notifyAll()` 唤醒；② 等待超时（`wait(毫秒数)`）自动唤醒；③ 被 `interrupt()` 中断。
